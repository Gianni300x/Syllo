...
const TAMANOS = {
  ...
  normal: {
    tile: TILE,
    panel: PANEL,
    raiz: "h-12 rounded-2xl",
    disparador: "h-12 w-12 rounded-2xl",
    circulo: "h-7 w-7",
    icono: 20,
  },
} as const;
...
const CIRCLE = `grid place-items-center rounded-full transition-colors duration-200 hover:bg-[#FAFAFD] dark:hover:bg-[#2C2C2C] ${FOCUS} ${SURFACE} ${LIFT}`;
...
> & {
  ...
  onCancel?: () => void;
  /** Variante compacta para acciones dentro de una tarjeta. */
  size?: "compacto" | "normal";
  disabled?: boolean;
  "aria-label"?: string;
};
...
export function DeleteButton({
  ...
  const confirmar = async () => {
    ...
    try {
      ...
      // Cada acción muestra su propio error junto al dato que intentó borrar.
    }
  };
  ...
  return (
    <motion.div
      ...
      data-status={status}
      className={cn("relative", tamano.raiz, SURFACE, GLYPH, className)}
      animate={{ width: open ? tamano.tile + tamano.panel : tamano.tile }}
    ...
    >
      ...
      <span role="status" aria-live="polite" className="sr-only">
        {status === "deleted" ? "Eliminado" : status === "kept" ? "Se canceló la eliminación" : ""}
      </span>
