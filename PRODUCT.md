...
usan el calendario del campus virtual / CVG. Uso
...
Syllo reúne en un solo panel la información dispersa de la cursada: los anuncios
de Google Classroom, las fechas del CVG y las tareas de todos los
...
reúne anuncios de Classroom + calendario del CVG + tareas + notas +
...
- Postgres (Neon) + Drizzle guardan lo que el usuario crea dentro de Syllo:
  ...
  propio de las tareas. Los anuncios nunca se persisten y de las tareas no se
  guarda el contenido: se piden a Google y se cachean unos minutos. La URL
  privada del CVG se guarda cifrada para poder actualizar sus fechas. La
...
- La app no solicita `gmail.readonly`; el acceso a Classroom es de solo lectura
  y la URL privada del CVG se valida y cifra antes de persistirla.
